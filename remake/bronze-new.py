...
# FIX [CRITICAL]: Dukascopy encodes all instruments as integer × 100_000.
# The previous value of 1_000 produced prices 100× too large.
    ...
    async def _fetch_hour(
        ...
        async with semaphore:
            for attempt in range(max_retries):
                try:
                    async with session.get(url) as resp:
                        ...
                        if resp.status == 503:
                            ...
                            logger.warning(
                                f"[HistoryDownloader] 503 for {hour_dt} "
                ...
                # Exhausted all retries (all were 503)
                async with lock:
                    summary["hours_skipped"] += 1
                return
